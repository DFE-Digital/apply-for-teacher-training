...
module "postgres_replica" {
  ...
  service_short                  = var.service_short
  config_short                   = var.config_short
  azure_name_override            = "${var.azure_resource_prefix}-${var.service_short}-${local.app_name_suffix}-psql-replica"
  cluster_configuration_map      = module.cluster_data.configuration_map
  use_azure                      = var.deploy_azure_backing_services
  pg_create_mode                 = "Replica"
  source_server_id               = module.postgres.azure_server_id
  azure_enable_monitoring        = var.enable_alerting
  azure_enable_backup_storage    = false
  server_version                 = var.postgres_replica_version
  admin_username                 = data.azurerm_key_vault_secret.postgres_admin_username.value
  admin_password                 = data.azurerm_key_vault_secret.postgres_admin_password.value
  azure_sku_name                 = var.postgres_replica_sku
  azure_storage_mb               = var.postgres_replica_storage_mb
  azure_enable_high_availability = var.replica_enable_high_availability
  azure_maintenance_window       = var.azure_maintenance_window
