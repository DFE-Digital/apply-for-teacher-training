...
RSpec.feature 'Candidate is redirected correctly' do
  ...
  def then_i_should_see_the_references_name_form
    expect(page).to have_current_path(
      candidate_interface_new_references_edit_name_path(@first_reference.id, return_to: 'application-review'),
    )
  end

  def then_i_should_see_the_references_type_form
    expect(page).to have_current_path(
      candidate_interface_new_references_edit_type_path(@first_reference.referee_type, @first_reference.id, return_to: 'application-review'),
    )
  end

  def then_i_should_see_the_references_email_form
    expect(page).to have_current_path(
      candidate_interface_new_references_edit_email_address_path(@first_reference.id, return_to: 'application-review'),
    )
  end

  def then_i_should_see_the_references_relationship_form
    expect(page).to have_current_path(
      candidate_interface_new_references_edit_relationship_path(@first_reference.id, return_to: 'application-review'),
    )
  end
