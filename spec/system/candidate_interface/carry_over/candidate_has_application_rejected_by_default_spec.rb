...
private
  ...
  def then_i_cannot_carry_over_my_application
    reject_by_default = @timetable.reject_by_default_at.to_fs(:govuk_date_time_time_first)
    expect(page).to have_current_path candidate_interface_application_choices_path
    expect(page).to have_content(
      "If you don’t hear back from training providers about your applications before #{reject_by_default} then they will be rejected automatically.",
    )
  end
