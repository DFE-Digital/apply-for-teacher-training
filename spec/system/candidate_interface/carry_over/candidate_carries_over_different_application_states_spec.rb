...
[RecruitmentCycle.current_year, RecruitmentCycle.previous_year].each do |year|
  RSpec.describe 'Carry over application to a new cycle in different states', time: CycleTimetableHelper.mid_cycle(year) do
    include CandidateHelper

    before do
      @candidate = create(:candidate)
      @application_form = create(:application_form, :completed, recruitment_cycle_year: year - 1, candidate: @candidate)
    end
