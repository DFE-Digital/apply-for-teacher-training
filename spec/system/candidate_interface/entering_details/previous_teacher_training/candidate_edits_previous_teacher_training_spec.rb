...
RSpec.describe 'Edit previous teacher training' do
  ...
  def then_i_am_on_the_review_page
    ...
    draft_record = @application_form.previous_teacher_trainings.draft.last
    summary_list = [
      {
        label: 'Have you started a teacher training course before?',
        value: draft_record.started.capitalize,
      },
      {
        label: 'Name of the training provider',
        value: draft_record.provider_name,
      },
      {
        label: 'Training dates',
        value: "From #{draft_record.started_at.to_fs(:month_and_year)} " \
               "to #{draft_record.ended_at.to_fs(:month_and_year)}",
      },
      {
        label: 'Details',
        value: draft_record.details,
      },
    ]
