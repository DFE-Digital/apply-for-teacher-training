...
RSpec.describe 'Previous teacher training' do
  ...
  def then_i_am_on_the_start_page
    expect(page).to have_current_path(
      new_candidate_interface_previous_teacher_training_start_path, ignore_query: true
    )
  end

  def then_i_am_on_the_provider_name_page
    expect(page).to have_current_path(
      edit_candidate_interface_previous_teacher_training_names_path, ignore_query: true
    )
  end
  ...
  def then_i_am_on_the_dates_page
    expect(page).to have_current_path(
      edit_candidate_interface_previous_teacher_training_dates_path, ignore_query: true
    )
  end

  def when_i_input_training_dates
    fill_in('previous_teacher_training_form_dates[started_at(2i)]', with: 1)
    fill_in('previous_teacher_training_form_dates[started_at(1i)]', with: 2005)
    fill_in('previous_teacher_training_form_dates[ended_at(2i)]', with: 1)
    fill_in('previous_teacher_training_form_dates[ended_at(1i)]', with: 2006)
  end

  def then_i_am_on_the_details_page
    expect(page).to have_current_path(
      edit_candidate_interface_previous_teacher_training_details_path, ignore_query: true
    )
  end
  ...
  def then_i_am_on_the_review_page
    expect(page).to have_current_path(
      candidate_interface_previous_teacher_training_review_path, ignore_query: true
    )
