...
RSpec.feature 'Entering their other qualifications', mid_cycle: false do
  ...
  def and_the_year_field_is_pre_populated_with_my_previous_details
    expect(page.find_by(id: 'candidate-interface-other-qualification-details-form-award-year-field').value).to eq('2015')

    # Test that the wizard data is cleared when starting a new qualification
    expect(page.find_by(id: 'candidate-interface-other-qualification-details-form-grade-field').value).to be_nil
  end
  ...
  def then_the_year_field_is_not_pre_populated_with_my_previous_details
    expect(page.find_by(id: 'candidate-interface-other-qualification-details-form-award-year-field').value).to be_nil
  end
