...
RSpec.describe 'New References', :with_audited, time: CycleTimetableHelper.after_apply_deadline do
  ...
  def and_i_be_on_start_page
    expect(page).to have_current_path(candidate_interface_request_new_reference_path('request-reference'))
  end

  def and_i_be_on_the_add_type_page
    expect(page).to have_current_path(
      candidate_interface_references_type_path('request-reference'),
    )
  end
