...
RSpec.describe 'Candidate selects secondary withdrawal reasons' do
  ...
  shared_examples_for 'Candidate selects a primary reason' do |primary_reason|
    scenario 'Candidate can navigate around the page after selecting', time: mid_cycle do
      ...
      when_i_select_the_primary_reason(primary_reason)
      then_i_see_the_correct_secondary_options_page(primary_reason)

      when_i_click_on('Back')
      then_i_am_on_the_start_page
      and_the_primary_reason_is_selected(primary_reason)
      ...
      when_i_click_on('Change primary reason for withdrawal')
  ...
  it_behaves_like(
    'Candidate selects a primary reason',
  ...
  def then_i_am_on_the_start_page
    expect(page).to have_current_path(
      candidate_interface_withdrawal_reasons_primary_reason_start_path(id: @application_choice.id),
      ignore_query: true,
    )
  end
  ...
  def and_the_primary_reason_is_selected(primary_reason)
    expect(find_field(primary_reason).checked?).to be true
  end

  def then_i_see_the_correct_secondary_options_page(primary_reason)
    key = reason_key_mapping[primary_reason]
    expect(page).to have_current_path(
      candidate_interface_withdrawal_reasons_secondary_reasons_start_path(id: @application_choice.id, primary_reason: key),
    )
    case key
