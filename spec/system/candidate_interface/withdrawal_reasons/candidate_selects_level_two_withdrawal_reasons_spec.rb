...
RSpec.describe 'Candidate selects level-two withdrawal reasons' do
  ...
  def then_i_am_on_the_start_page
    expect(page).to have_current_path(
      candidate_interface_withdrawal_reasons_level_one_reason_start_path(id: @application_choice.id),
      ignore_query: true,
    )
  end
  ...
  def then_i_see_the_correct_level_two_options_page(level_one_reason)
    ...
    expect(page).to have_current_path(
      candidate_interface_withdrawal_reasons_level_two_reasons_start_path(id: @application_choice.id, level_one_reason: key),
    )
    case key
