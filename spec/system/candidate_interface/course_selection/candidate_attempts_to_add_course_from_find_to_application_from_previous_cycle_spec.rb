...
RSpec.describe 'Candidate attempts to add course via Find to application from previous cycle' do
  ...
  def then_i_see_that_my_application_must_be_carried_over
    expect(page).to have_text('You started an application for courses starting in the 2020 to 2021 academic year, which have now closed.')
    expect(page).to have_text("Continue your application to apply for courses starting in the #{current_year} to #{next_year} academic year instead.")
