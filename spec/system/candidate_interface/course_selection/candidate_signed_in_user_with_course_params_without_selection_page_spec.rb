...
RSpec.describe 'An existing candidate arriving from Find with a course and provider code' do
  ...
  def then_i_should_see_the_course_choices_site_page
    expect(page).to have_current_path(candidate_interface_course_choices_site_path(@course_with_multiple_sites.provider.id, @course_with_multiple_sites.id, @course_with_multiple_sites.study_mode))
  end
