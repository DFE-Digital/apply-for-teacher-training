...
RSpec.feature 'New References' do
  ...
  around do |example|
    TestSuiteTimeMachine.travel_temporarily_to(CycleTimetable.apply_1_deadline(ApplicationForm::OLD_REFERENCE_FLOW_CYCLE_YEAR) + 1.day) do
      example.run
    end
  end
  ...
  def then_i_see_the_type_page
    expect(page).to have_current_path candidate_interface_new_references_type_path
  end
  ...
  def then_i_should_see_the_referee_name_page
    expect(page).to have_current_path candidate_interface_new_references_name_path('academic')
  end
  ...
  def then_i_see_the_referee_email_page
    expect(page).to have_current_path candidate_interface_new_references_email_address_path(@application.application_references.last.id)
  end
  ...
  def then_i_see_the_relationship_page
    expect(page).to have_current_path candidate_interface_new_references_relationship_path(@application.application_references.last.id)
  end
  ...
  def when_i_try_to_edit_someone_elses_reference
    ...
    visit candidate_interface_new_references_edit_name_path(non_associated_reference.id)
  end

  def when_i_try_and_edit_a_reference_that_does_not_exist
    visit candidate_interface_new_references_edit_name_path('INVALID')
  end
