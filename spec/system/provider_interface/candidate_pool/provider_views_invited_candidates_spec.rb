...
RSpec.describe 'Invited candidate list' do
  ...
  let(:invite_with_application) do
    create(
      ...
      candidate: submitted_application_choice.application_form.candidate,
    )
  end
  ...
  let(:invite_with_opted_out_candidate) { create(:pool_invite, :sent_to_candidate, course: build(:course, provider: current_provider), candidate: opted_out_preference.candidate) }
  ...
  let(:invite_with_candidate_in_pool) { create(:pool_invite, :sent_to_candidate, candidate: opted_in_preference.candidate, course: build(:course, provider: current_provider)) }
  let(:second_invite_with_candidate_in_pool) { create(:pool_invite, :sent_to_candidate, candidate: opted_in_preference.candidate, course: build(:course, provider: current_provider)) }
...
private
  ...
  def and_i_have_an_existing_filter
    create(
      ...
      path: provider_interface_candidate_pool_invites_path,
