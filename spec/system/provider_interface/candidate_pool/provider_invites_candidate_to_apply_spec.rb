...
RSpec.describe 'Providers invites candidates' do
  ...
  def when_i_visit_the_candidate_pool_show_page
    visit provider_interface_find_candidate_path(@candidate)
  end
  ...
  def then_i_am_redirected_to_the_new_invite_form
    expect(page).to have_current_path(provider_interface_new_candidate_pool_invite_path(@candidate), ignore_query: true)
  end
  ...
  def then_i_am_redirected_to_the_review_page(course)
    ...
    expect(page).to have_current_path(provider_interface_show_candidate_pool_invite_path(pool_invite), ignore_query: true)
  ...
  def then_i_am_redirected_to_the_edit_page
    ...
    expect(page).to have_current_path(
      provider_interface_edit_candidate_pool_invite_path(@candidate, pool_invite),
      ignore_query: true,
    )
  end
  ...
  def then_i_am_on_the_candidate_pool_page(course)
    expect(page).to have_current_path(
      provider_interface_find_candidates_path,
      ignore_query: true,
    )
