...
RSpec.feature 'Provider reinstates deferred offer' do
  ...
  let(:course_current_year) { create(:course, provider: provider) }
  let(:course_previous_year) { create(:course, :previous_year, provider: provider) }
  let(:course_previous_year_but_still_available) { create(:course, :previous_year_but_still_available, provider: provider) }
  ...
  def then_i_can_see_the_details_of_the_deferred_offer
    expect(page).to have_content choices[:reinstatable].offered_option.site.name_and_address
  end
  ...
  def and_the_course_now_offered_is_from_the_current_year
    expect(choices[:reinstatable].reload.offered_course.recruitment_cycle_year).to eq(RecruitmentCycle.current_year)
  end
end
