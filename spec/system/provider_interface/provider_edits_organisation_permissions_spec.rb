...
RSpec.describe 'Provider edits organisation permissions' do
  ...
  def and_i_click_on_a_particular_organisation_permissions_link
    click_link_or_button "Organisation permissions #{@ratifying_provider.name}"
  end

  def and_i_click_on_organisation_permissions
    click_link_or_button 'Organisation permissions'
  end

  def and_i_click_on_an_organisation_i_can_manage
    within('.app-application-card h2') do
      click_link_or_button @ratifying_provider.name
    end
  end
