...
RSpec.describe 'Generating new data exports', :with_audited do
  ...
  scenario 'When the export type has been deprecated' do
    given_i_am_signed_in_as_a_support_user
    and_i_go_to_the_data_export_page
    then_i_do_not_see_the_deprecated_report_type
  end
...
private
  ...
  def then_i_do_not_see_the_deprecated_report_type
    deprecated_export_type_names = DataExport.deprecated_export_types.values.map { |et| et[:name] }

    deprecated_export_type_names.each do |name|
      within('.govuk-main-wrapper') do
