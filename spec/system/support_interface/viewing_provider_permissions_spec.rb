...
RSpec.describe 'Viewing provider-provider permissions via support' do
  ...
  def and_there_are_two_other_providers_in_a_partnership_with_no_permissions_configured
    training = create(:provider, name: 'City Learning Trust')
    ratifying = create(:provider, name: 'Staffordshire University (S72)')
  ...
  def then_i_see_the_training_provider_permissions_diagram
    expect(page).to have_text 'can ✔️ view safeguarding ❌ view diversity ❌ make decisions for courses ratified by'
  end
  ...
  def then_i_see_the_ratifying_provider_permissions_diagram
    expect(page).to have_text 'can ❌ view safeguarding ✔️ view diversity ✔️ make decisions for courses run by'
  end
  ...
  def then_i_clearly_see_that_no_permissions_have_been_setup
    expect(page).to have_text 'Permissions not setup'
  end
end
