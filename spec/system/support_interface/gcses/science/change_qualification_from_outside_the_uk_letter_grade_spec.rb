...
RSpec.describe 'Change science GCSE' do
  ...
  scenario 'Change the science qualification from outside the UK (letter grade) on an application form', :with_audited do
...
private
  ...
  def then_i_see_the_form_to_change_the_applications_science_international_qualification
    ...
    structured_grades.each do |grade|
      expect(page).to have_field(grade, type: :radio, checked: grade == @science_gcse.grade)
    end
