...
RSpec.describe 'Change maths GCSE' do
  ...
  scenario 'Change the maths qualification from outside the UK (letter grade) on an application form', :with_audited do
    given_i_am_a_support_user
    and_there_is_an_application_choice_awaiting_provider_decision

    when_i_visit_the_application_page
    and_i_click_to_change_the_maths_international_qualification
    then_i_see_the_form_to_change_the_applications_maths_international_qualification
    ...
    and_the_maths_international_qualification_has_been_updated
  end
...
private
  ...
  def and_there_is_an_application_choice_awaiting_provider_decision
    ...
    @maths_gcse = create(
      ...
      selected_grade_schema_id: '28c6a18a-2040-43af-8cf2-83b7e14ba14c',
    )
  ...
  def and_i_click_to_change_the_maths_international_qualification
    within('.app-edit-qualification') do
      click_link_or_button 'Change'
    end
  end

  def then_i_see_the_form_to_change_the_applications_maths_international_qualification
    ...
    structured_grades.each do |grade|
      expect(page).to have_field(grade, type: :radio, checked: grade == @maths_gcse.grade)
    end
    ...
    expect(page).to have_field('UK ENIC reference number', with: @maths_gcse.enic_reference)
  ...
  def and_the_maths_international_qualification_has_been_updated
    expect(page).to have_element(:h4, text: 'Maths CBSE Class 10 (AISSE)')
