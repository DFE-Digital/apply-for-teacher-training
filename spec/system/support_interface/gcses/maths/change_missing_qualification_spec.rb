...
RSpec.describe 'Change maths GCSE' do
  ...
  scenario 'Change the missing maths GCSE details on an application form', :with_audited do
    given_i_am_a_support_user
    and_there_is_an_application_choice_awaiting_provider_decision

    when_i_visit_the_application_page
    and_i_click_to_change_the_missing_maths_gcse
    then_i_see_the_form_to_change_the_applications_missing_maths_gcse
    ...
    and_the_missing_maths_gcse_details_have_been_updated

    when_i_click_to_change_the_missing_maths_gcse
    and_i_select_no_to_studying_a_gcse
    and_i_enter_other_evidence_of_having_maths_skills
    and_i_enter_a_zendesk_link
    and_click_on_update_details
    and_the_missing_maths_gcse_details_have_been_updated_with_other_evidence
  end
...
private
  ...
  def and_i_click_to_change_the_missing_maths_gcse
    within('.app-edit-qualification') do
      click_link_or_button 'Change'
    end
  end
  alias_method :when_i_click_to_change_the_missing_maths_gcse, :and_i_click_to_change_the_missing_maths_gcse

  def then_i_see_the_form_to_change_the_applications_missing_maths_gcse
    expect(page).to have_element(:h1, text: 'Edit maths GCSE or equivalent')

    expect(page).to have_element(:legend, text: 'What type of qualification in maths do you have?')
    ...
    expect(page).to have_field('I do not have a qualification in maths yet', type: 'radio', checked: true)

    expect(page).to have_element(:legend, text: 'Are you currently studying for a GCSE in maths, or equivalent?')
    expect(page).to have_field('Yes', type: 'radio', checked: true)
    expect(page).to have_field('Details of the qualification you are studying for', with: @maths_gcse.not_completed_explanation)
    expect(page).to have_field('No', type: 'radio', checked: false)
    expect(page).to have_field(
      'If you have other evidence of having maths skills at the required standard, give details (optional)',
      visible: :all,
    )
  ...
  def and_the_missing_maths_gcse_details_have_been_updated
    expect(page).to have_element(:h4, text: 'Maths')
  ...
  def and_i_enter_other_evidence_of_having_maths_skills
    fill_in 'If you have other evidence of having maths skills at the required standard, give details (optional)',
            with: 'I have a calculator in my brain'
  end

  def and_the_missing_maths_gcse_details_have_been_updated_with_other_evidence
    expect(page).to have_element(:h4, text: 'Maths')
    expect(page).to have_element(:dd, text: 'Candidate does not have this qualification yet')
