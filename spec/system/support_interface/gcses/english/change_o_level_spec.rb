...
RSpec.describe 'Change english GCSE' do
  ...
  scenario 'Change the english O level on an application form', :with_audited do
    given_i_am_a_support_user
    and_there_is_an_application_choice_awaiting_provider_decision

    when_i_visit_the_application_page
    and_i_click_to_change_the_english_o_level
    then_i_see_the_form_to_change_the_applications_english_o_level
...
private
  ...
  def and_i_click_to_change_the_english_o_level
    within('.app-edit-qualification') do
      click_link_or_button 'Change'
    end
  end

  def then_i_see_the_form_to_change_the_applications_english_o_level
  ...
  def and_the_english_o_level_has_been_updated
    expect(page).to have_element(:h4, text: 'English UK O level (from before 1989)')
