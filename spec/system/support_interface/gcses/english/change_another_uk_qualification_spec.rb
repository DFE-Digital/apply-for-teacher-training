...
RSpec.describe 'Change english GCSE' do
  ...
  scenario 'Change another english uk qualification (not GCSE) on an application form', :with_audited do
    given_i_am_a_support_user
    and_there_is_an_application_choice_awaiting_provider_decision

    when_i_visit_the_application_page
    and_i_click_to_change_the_other_english_qualification
    then_i_see_the_form_to_change_the_applications_other_english_qualification
    ...
    and_the_other_english_qualification_has_been_updated
  end
...
private
  ...
  def and_there_is_an_application_choice_awaiting_provider_decision
    ...
    @english_gcse = create(
      ...
      other_uk_qualification_type: 'Another English Qualification',
  ...
  def and_i_click_to_change_the_other_english_qualification
    within('.app-edit-qualification') do
      click_link_or_button 'Change'
    end
  end

  def then_i_see_the_form_to_change_the_applications_other_english_qualification
    ...
    expect(page).to have_field('Qualification name', with: 'Another English Qualification')
    expect(page).to have_field('Grade', with: @english_gcse.grade)
  ...
  def and_the_other_english_qualification_has_been_updated
    expect(page).to have_element(:h4, text: 'English Some other qualification')
