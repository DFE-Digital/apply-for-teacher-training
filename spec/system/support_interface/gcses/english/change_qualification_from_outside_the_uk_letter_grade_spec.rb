...
RSpec.describe 'Change english GCSE' do
  ...
  scenario 'Change the english qualification from outside the UK (letter grade) on an application form', :with_audited do
    given_i_am_a_support_user
    and_there_is_an_application_choice_awaiting_provider_decision

    when_i_visit_the_application_page
    and_i_click_to_change_the_english_international_qualification
    then_i_see_the_form_to_change_the_applications_english_international_qualification
    ...
    and_the_english_international_qualification_has_been_updated
  end
...
private
  ...
  def and_there_is_an_application_choice_awaiting_provider_decision
    ...
    @english_gcse = create(
      ...
      selected_grade_schema_id: 'bad86f85-e46d-413f-8d6c-525e6ecc0d8b',
    )
  ...
  def and_i_click_to_change_the_english_international_qualification
    within('.app-edit-qualification') do
      click_link_or_button 'Change'
    end
  end

  def then_i_see_the_form_to_change_the_applications_english_international_qualification
    ...
    structured_grades.each do |grade|
      expect(page).to have_field(grade, type: :radio, checked: grade == @english_gcse.grade)
    end
    ...
    expect(page).to have_field('UK ENIC reference number', with: @english_gcse.enic_reference)
  ...
  def structured_grades
    finder = InternationalQualifications::StructuredGcseOptionFinder.new(
      @english_gcse.institution_country,
      @english_gcse.subject,
    )
    selected_equivalent_qualification = finder.equivalent_qualifications.find do |qualification|
      qualification.name == @english_gcse.non_uk_qualification_type
    end
  ...
  def and_the_english_international_qualification_has_been_updated
    expect(page).to have_element(:h4, text: 'English CBSE Class 10 (AISSE)')
    ...
    expect(page).to have_element(:dd, text: 'B1')
  end
end
