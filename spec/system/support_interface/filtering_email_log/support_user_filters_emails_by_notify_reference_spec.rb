...
RSpec.describe 'Support user filters emails by notify reference' do
  ...
  scenario do
    ...
    and_i_filter_emails_by_notify_reference
  ...
  def and_an_emails_has_been_dispatched
    @bob_email = create(:email, to: 'bob@example.com', subject: 'Hello Bob', notify_reference: 'ABC123')
    @jane_email = create(:email, to: 'jane@example.com', subject: 'Hello Jane', notify_reference: 'XYZ789')
  end
  ...
  def and_i_filter_emails_by_notify_reference
    within('.moj-filter__content') do
      fill_in 'Notify reference', with: 'ABC123'
