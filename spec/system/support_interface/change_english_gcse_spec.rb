...
RSpec.describe 'Change english GCSE' do
  ...
  scenario 'Change the english GCSE on an application form', :with_audited do
    ...
    when_i_check_all_gcses
    and_i_click_update
    then_i_see_a_validation_error_about_gcse_grades
  ...
  def and_i_add_the_candidate_is_not_studying_for_gcse
    choose 'No'
    fill_in 'If you have other evidence of having English skills at the required standard, give details (optional)', with: 'Many reasons listed'
  end
  ...
  def when_i_add_the_grades
    fill_in 'support_interface_gcse_form[grade_english_single]', with: '4'
    fill_in 'support_interface_gcse_form[grade_english_double]', with: '1-1'
    fill_in 'support_interface_gcse_form[grade_english_language]', with: 'A'
    fill_in 'support_interface_gcse_form[grade_english_literature]', with: 'B'
    fill_in 'support_interface_gcse_form[grade_english_studies_single]', with: 'C'
    fill_in 'support_interface_gcse_form[grade_english_studies_double]', with: 'A*A'
    fill_in 'support_interface_gcse_form[other_english_gcse_name]', with: 'Awesome name'
    fill_in 'support_interface_gcse_form[grade_other_english_gcse]', with: '3'
    and_i_add_the_zendesk_ticket_url
  end
  ...
  def then_it_has_saved_missing_qualification_into_the_application_form
    ...
    @english_gcse.reload
    expect(@english_gcse.attributes.symbolize_keys).to include(
  ...
  def then_it_has_saved_gcses_into_the_application_form
    ...
    expect(@english_gcse.constituent_grades['english_language']).to include(
      'grade' => 'A',
    )
    expect(@english_gcse.constituent_grades['english_literature']).to include(
      'grade' => 'B',
    )
    expect(@english_gcse.constituent_grades['english_double_award']).to include(
      'grade' => '1-1',
    )
    expect(@english_gcse.constituent_grades['english_single_award']).to include(
      'grade' => '4',
    )
    expect(@english_gcse.constituent_grades['english_studies_double_award']).to include(
      'grade' => 'A*A',
    )
    expect(@english_gcse.constituent_grades['english_studies_single_award']).to include(
      'grade' => 'C',
    )
    expect(@english_gcse.constituent_grades['Awesome name']).to include(
      'grade' => '3',
    )
  end
end
