...
RSpec.describe 'Editing address' do
  ...
  before do
    allow(Pagy).to receive(:new).and_return(
      Pagy.new(count: 1, page: 1, items: 100),
    )
  end
  ...
  def when_i_complete_the_details_form
    ...
    fill_in 'support_interface_application_forms_edit_address_details_form[audit_comment]', with: 'Updated as part of Zendesk ticket #12345'
  end
  ...
  def when_i_fill_in_an_international_address
    ...
    fill_in 'support_interface_application_forms_edit_address_details_form[audit_comment]', with: 'Updated as part of Zendesk ticket #56789'
  end
