...
RSpec.describe 'Send notify template' do
  ...
  scenario 'Support user uploads a malformed CSV as a distribution list' do
    ...
    and_i_click_on_send
    then_i_see_an_error_for_uploading_a_malformed_csv_distribution_list
  end
...
private
  ...
  def when_i_complete_the_notify_template_form
    ...
    attach_file 'Distribution list', 'spec/fixtures/send_notify_template/malformed_distribution_list.csv'
  ...
  def then_i_see_an_error_for_uploading_a_malformed_csv_distribution_list
    within('.govuk-error-summary') do
      expect(page).to have_element(:h2, text: 'There is a problem', class: 'govuk-error-summary__title')
      expect(page).to have_element(:div, text: 'Distribution list CSV is malformed, check the content of the file', class: 'govuk-error-summary__body')
    end
  end
end
