...
RSpec.describe 'Send notify template' do
  ...
  scenario 'Support user uploads an attachment with is too large' do
    ...
    and_i_click_on_send
    then_i_see_an_error_for_uploading_an_attachment_which_is_too_large
  end
...
private
  ...
  def then_i_see_an_error_for_uploading_an_attachment_which_is_too_large
    within('.govuk-error-summary') do
      expect(page).to have_element(:h2, text: 'There is a problem', class: 'govuk-error-summary__title')
      expect(page).to have_element(
        :div,
        text: 'The file attachment must be 2MB or smaller.',
