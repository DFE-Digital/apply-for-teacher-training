...
RSpec.describe 'Send notify template' do
  ...
  scenario 'Support user does not upload an attachment' do
    ...
    and_i_click_on_send
    then_i_see_an_error_for_not_uploading_an_attachment
  end
...
private
  ...
  def then_i_see_an_error_for_not_uploading_an_attachment
    within('.govuk-error-summary') do
      expect(page).to have_element(:h2, text: 'There is a problem', class: 'govuk-error-summary__title')
      expect(page).to have_element(:div, text: 'Upload a file attachment', class: 'govuk-error-summary__body')
    end
  end
end
