...
RSpec.describe 'Send notify template' do
  ...
  scenario 'Support user uploads a distribution list with invalid headers' do
    ...
    and_i_click_on_send
    then_i_see_an_error_for_uploading_a_distribution_list_with_invalid_headers
  end
...
private
  ...
  def when_i_complete_the_notify_template_form
    ...
    attach_file 'Distribution list', 'spec/fixtures/send_notify_template/distribution_list_with_invalid_headers.csv'
  ...
  def then_i_see_an_error_for_uploading_a_distribution_list_with_invalid_headers
    within('.govuk-error-summary') do
      expect(page).to have_element(:h2, text: 'There is a problem', class: 'govuk-error-summary__title')
      expect(page).to have_element(:div, text: 'Distribution list must include the column header Email address.', class: 'govuk-error-summary__body')
    end
  end
end
