...
RSpec.describe ViewHelper, type: :helper do
  ...
  describe '#date_and_time_today_or_tomorrow' do
    it 'returns the time tomorrow for a time tomorrow' do
      time = Time.zone.tomorrow.midnight + 3.hours
      expect(helper.date_and_time_today_or_tomorrow(time)).to eq "tomorrow (#{time.to_fs(:govuk_date_and_time)})"
    end
  ...
  describe '#govuk_link_to_with_utm_params' do
    context 'GIT link with utm params from application' do
      ...
      let(:utm_campaign) { 'reference/edit' }
      ...
      it 'returns govuk_link_to html with utm params' do
        ...
        expect(link).to eq('<a class="govuk-link" href="/some/link?utm_source=apply-for-teacher-training.service.gov.uk&amp;utm_medium=referral&amp;utm_campaign=reference/edit&amp;utm_content=apply_1">Somewhere over the rainbow</a>')
  ...
  describe '#utm_campaign' do
    context 'when route is namespaced' do
      let(:path) { '/candidate/application/degree/new_enic' }

      it 'removes /candidate/application to shorten utm parameter' do
        utm_campaign = helper.utm_campaign(path)

        expect(utm_campaign).to eq('/degree/new_enic')
      end
    end

    context 'when route is not namespaced' do
      let(:path) { '/candidate/application/complete' }
      let(:context) { 'apply_again' }

      it 'removes /candidate/application and adds context' do
        utm_campaign = helper.utm_campaign(path, context)

        expect(utm_campaign).to eq('/complete-apply_again')
      end
    end
  end
end
