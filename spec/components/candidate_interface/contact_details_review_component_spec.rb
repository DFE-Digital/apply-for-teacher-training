...
RSpec.describe CandidateInterface::ContactDetailsReviewComponent, type: :component do
  ...
  context 'when residency since birth is true' do
    let(:application_form) do
      build_stubbed(
        :application_form,
        phone_number: '07700 900 982',
    ...
    it 'renders the residency row without the dates row' do
      ...
      expect(result.css('.govuk-summary-list__key').text).to include('Have you lived in France since birth?')
      expect(result.css('.govuk-summary-list__value').text).to include('Yes')
      expect(result.css('.govuk-summary-list__key').text).not_to include('From what date have you lived in France?')
    end
  end

  context 'when residency since birth is false and a residency start date has been provided' do
    let(:application_form) do
      build_stubbed(
        :application_form,
        phone_number: '07700 900 982',
