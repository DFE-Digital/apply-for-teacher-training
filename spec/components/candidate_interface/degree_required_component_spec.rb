...
RSpec.describe CandidateInterface::DegreeRequiredComponent, type: :component do
  ...
  let(:course_option) { create(:course_option, course: create(:course, :open, degree_grade: 'two_one')) }
  ...
  context 'application has uk degree and but degree grade marked as not_required' do
    let(:course_option) { create(:course_option, course: create(:course, :open, degree_grade: 'not_required')) }
    let(:application_choice) do
      build_stubbed(
        :application_choice,
        status: :unsubmitted,
