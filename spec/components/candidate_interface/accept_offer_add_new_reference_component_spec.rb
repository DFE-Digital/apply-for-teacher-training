...
RSpec.describe CandidateInterface::AcceptOfferAddNewReferenceComponent do
  it 'add focus link' do
    ...
    result = render_inline(described_class.new(application_form: application_form, application_choice: application_choice))
  ...
  context 'when application has two references' do
    it 'renders secondary button' do
      application_form = create(:application_form)
      create_list(:reference, 2, :not_requested_yet, application_form: application_form)
      application_choice = create(:application_choice, application_form: application_form)
      result = render_inline(described_class.new(application_form: application_form, application_choice: application_choice))
