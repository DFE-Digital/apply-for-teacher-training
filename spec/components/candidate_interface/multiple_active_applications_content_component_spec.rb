...
RSpec.describe CandidateInterface::MultipleActiveApplicationsContentComponent do
  ...
  describe 'rendered when the candidate has previous active applications' do
    ...
    context 'when before the winter reject by default date' do
      before do
        TestSuiteTimeMachine.travel_permanently_to(previous_application.recruitment_cycle_timetable.winter_reject_by_default_at - 1.day)
      end
  ...
  context 'rendered when the candidate does not have previous active applications' do
    subject(:rendered) { component.render? }
