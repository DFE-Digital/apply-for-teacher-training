...
RSpec.describe CycleTimetable do
  ...
  describe '.current_year' do
    it 'is 2020 if we are in the middle of the 2020 cycle' do
      TestSuiteTimeMachine.travel_temporarily_to(one_hour_after_2020_cycle_opens) do
        expect(described_class.current_year).to eq(2020)
      end
    end

    it 'is 2021 if we are in the middle of the 2021 cycle' do
      TestSuiteTimeMachine.travel_temporarily_to(one_hour_after_2021_cycle_opens) do
  ...
  describe '.show_apply_1_deadline_banner?' do
    it 'returns true before the configured date and it is an unsuccessful apply_1 application' do
      ...
      TestSuiteTimeMachine.travel_temporarily_to(one_hour_before_apply1_deadline) do
    ...
    it 'returns false after the configured date' do
      ...
      TestSuiteTimeMachine.travel_temporarily_to(one_hour_after_apply1_deadline) do
        expect(described_class.show_apply_1_deadline_banner?(application_form)).to be false
      end
    end
  end
  ...
  describe '.show_summer_recruitment_banner?' do
    it 'returns false before the configured date' do
      TestSuiteTimeMachine.travel_temporarily_to(one_hour_before_show_summer_recruitment_banner) do
    ...
    it 'returns true between configure date and apply 1 closes' do
      TestSuiteTimeMachine.travel_temporarily_to(one_hour_after_show_summer_recruitment_banner) do
        expect(described_class.show_summer_recruitment_banner?).to be true
      end
    end

    it 'returns false after apply 1 closes' do
      TestSuiteTimeMachine.travel_temporarily_to(described_class.apply_1_deadline(2022) + 1.hour) do
        expect(described_class.show_summer_recruitment_banner?).to be false
      end
    end
  end
  ...
  describe '.show_apply_2_deadline_banner?' do
    it 'returns true before the configured date and it is a phase 2 application' do
      ...
      TestSuiteTimeMachine.travel_temporarily_to(one_hour_before_apply2_deadline) do
    ...
    it 'returns false after the configured date' do
      ...
      TestSuiteTimeMachine.travel_temporarily_to(one_hour_after_apply2_deadline) do
  ...
  describe '.show_non_working_days_banner?' do
    context 'when within the Christmas period' do
      ...
      it 'returns true if after the 20 day period' do
        TestSuiteTimeMachine.travel_temporarily_to(twenty_days_after_2021_cycle_opens) do
          expect(described_class.show_non_working_days_banner?).to be true
        end
      end

      it 'returns false after the Christmas period' do
        TestSuiteTimeMachine.travel_temporarily_to(one_hour_after_christmas_period) do
          expect(described_class.show_non_working_days_banner?).to be false
        end
      end
    end
    ...
    context 'when within the Easter period' do
      ...
      it 'returns false if before the holiday period' do
        TestSuiteTimeMachine.travel_temporarily_to(eleven_days_before_easter_period) do
          expect(described_class.show_non_working_days_banner?).to be false
        end
      end

      it 'returns true if within holiday period' do
        TestSuiteTimeMachine.travel_temporarily_to(within_easter_period) do
          expect(described_class.show_non_working_days_banner?).to be true
        end
      end

      it 'returns false after the holiday period' do
        TestSuiteTimeMachine.travel_temporarily_to(one_hour_after_easter_period) do
          expect(described_class.show_non_working_days_banner?).to be false
        end
      end
    end
  ...
  describe '.between_cycles_apply_1?' do
    ...
    it 'returns true between find and apply reopening' do
      TestSuiteTimeMachine.travel_temporarily_to(one_hour_after_find_opens) do
  ...
  describe '.find_down?' do
    it 'returns false before find closes' do
      TestSuiteTimeMachine.travel_temporarily_to(one_hour_before_find_closes) do
    ...
    it 'returns true between find_closes and find_reopens' do
      TestSuiteTimeMachine.travel_temporarily_to(one_hour_after_find_closes) do
  ...
  describe '.days_until_find_reopens' do
    it 'returns the number of days until Find reopens' do
      TestSuiteTimeMachine.travel_temporarily_to(three_days_before_find_reopens) do
  ...
  describe 'can_add_course_choice?' do
    ...
    context 'application form is from a previous recruitment cycle' do
      ...
      it 'returns false' do
        TestSuiteTimeMachine.travel_temporarily_to(described_class.apply_opens) do
          expect(execute_service).to be false
        end
      end
    end
  ...
  describe '.need_to_send_deadline_reminder?' do
    it 'does not return for a non deadline date' do
      TestSuiteTimeMachine.travel_temporarily_to(described_class.apply_1_deadline_first_reminder - 1.day) do
    ...
    it 'returns apply_1 when it is the first apply 1 deadline' do
      TestSuiteTimeMachine.travel_temporarily_to(described_class.apply_1_deadline_first_reminder) do
    ...
    it 'returns apply_1 when it is the second apply 1 deadline' do
      TestSuiteTimeMachine.travel_temporarily_to(described_class.apply_1_deadline_second_reminder) do
        expect(described_class.need_to_send_deadline_reminder?).to be :apply_1
      end
    end

    it 'returns apply_2 when it is the first apply 2 deadline' do
      TestSuiteTimeMachine.travel_temporarily_to(described_class.apply_2_deadline_first_reminder) do
    ...
    it 'returns apply_2 when it is the second apply 2 deadline' do
      TestSuiteTimeMachine.travel_temporarily_to(described_class.apply_2_deadline_second_reminder) do
        expect(described_class.need_to_send_deadline_reminder?).to be :apply_2
      end
    end
  end
  ...
  describe 'apply_1_deadline_has_passed?' do
    ...
    context 'it is after the apply 1 deadline' do
      it 'returns true' do
        TestSuiteTimeMachine.travel_temporarily_to(described_class.apply_2_deadline) do
          application_form = build(:application_form)
  ...
  describe 'apply_2_deadline_has_passed?' do
    ...
    context 'it is after the apply 1 deadline' do
      it 'returns true' do
        TestSuiteTimeMachine.travel_temporarily_to(described_class.find_closes) do
          application_form = build(:application_form)
          expect(described_class.apply_1_deadline_has_passed?(application_form)).to be(true)
        end
      end
  ...
  describe '.send_find_has_opened_email?' do
    context 'it is before find reopens' do
      it 'returns false' do
        TestSuiteTimeMachine.travel_temporarily_to(described_class.find_opens - 1.hour) do
  ...
  describe '.service_opens_today?' do
    ...
    it 'is true when the service is Apply and the time is within business hours' do
      TestSuiteTimeMachine.travel_temporarily_to(1.minute.since(described_class.apply_opens(year))) do
    ...
    it 'is false when the service is Apply and the time is outside of business hours' do
      TestSuiteTimeMachine.travel_temporarily_to(12.hours.since(described_class.apply_opens(year))) do
    ...
    it 'is true when the service is Find and the time is within business hours' do
      TestSuiteTimeMachine.travel_temporarily_to(1.minute.since(described_class.find_opens(year))) do
    ...
    it 'is false when the service is Find and the time is outside of business hours' do
      TestSuiteTimeMachine.travel_temporarily_to(12.hours.since(described_class.find_opens(year))) do
  ...
  describe '.send_new_cycle_has_started_email?' do
    context 'it is before apply reopens' do
      it 'returns false' do
        TestSuiteTimeMachine.travel_temporarily_to(described_class.apply_reopens - 1.day) do
  ...
  describe '.between_reject_by_default_and_find_reopens?' do
    context 'it is before reject by default date' do
      it 'returns false' do
        TestSuiteTimeMachine.travel_temporarily_to(described_class.reject_by_default - 1.day) do
