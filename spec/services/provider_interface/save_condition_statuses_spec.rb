...
RSpec.describe ProviderInterface::SaveConditionStatuses do
  ...
  describe 'save!' do
    ...
    context 'when none of the conditions are marked as unmet' do
      context 'when all conditions are met' do
        ...
        it 'sends an email to the candidate', :sidekiq do
          service.save!
          expect(ActionMailer::Base.deliveries.first['rails-mail-template'].first.value).to eq('conditions_met')
        end
    ...
    context 'when one of the conditions is marked as unmet' do
      ...
      it 'sends an email to the candidate', :sidekiq do
        service.save!
        expect(ActionMailer::Base.deliveries.first['rails-mail-template'].first.value).to eq('conditions_not_met')
      end
