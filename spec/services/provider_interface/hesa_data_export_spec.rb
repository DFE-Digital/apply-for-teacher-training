...
RSpec.describe ProviderInterface::HesaDataExport do
  describe '#call' do
    ...
    subject(:export_data) { described_class.new(export_options: export_options).call }

    it 'generates CSV headers' do
      expect(export_data).to eq([
    ...
    context 'exported applications data' do
      let(:export_options) { { include_diversity_information: true } }
