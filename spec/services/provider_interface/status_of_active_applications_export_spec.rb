...
RSpec.describe ProviderInterface::StatusOfActiveApplicationsExport do
  ...
  before do
    ...
    create_list(:application_choice, 1, status: :awaiting_provider_decision, course_option: course_option_with_other_accredited_provider)
    create_list(:application_choice, 2, status: :inactive, course_option: course_option_with_other_accredited_provider)
  end
  ...
  describe '#call' do
    it 'outputs the rows in the correct format' do
      ...
      expect(row[I18n.t('provider_application_states.awaiting_provider_decision').to_s]).to eq('3')
    ...
    it 'outputs the total of each column' do
      ...
      expect(parsed_data.last[3]).to eq('3')
