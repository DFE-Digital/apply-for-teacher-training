...
RSpec.describe RedactApplication do
  ...
  describe '#call!', :with_audited do
    context 'when the application form has inflight application choices' do
      %i[awaiting_provider_decision interviewing inactive pending_conditions conditions_not_met recruited offer_deferred].each do |status|
        it "raises an error if application has been an application choice with the status #{status}" do
          create(:application_choice, status: status, application_form:)
          expect { service.call! }.to raise_error(
            'Application has inflight applications. You will need to withdrawn the applications first!',
          )
        end
      end
    end

    context 'when the application form has no inflight application choices' do
      %i[unsubmitted cancelled offer rejected application_not_sent offer_withdrawn declined withdrawn].each do |status|
        it "does not raise an error if application has been an application choice with the status #{status}" do
          create(:application_choice, status: status, application_form:)
    ...
    it 'does not remove their application choices' do
      ...
      application_form.reload
      expect(application_form.application_choices.count).to eq(1)
    end

    it 'does not removes associations' do
      service.call!

      application_form.reload
      DeleteApplication::ASSOCIATIONS_TO_DESTROY.each do |assoc|
        expect(application_form.send(assoc).count).not_to be_zero
      end
    end
    ...
    it 'not to delete the candidate_pool_application' do
      candidate_pool_application = create(:candidate_pool_application, application_form:)

      service.call!

      expect(application_form.candidate_pool_application).to eq(candidate_pool_application)
    end
