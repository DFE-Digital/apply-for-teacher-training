...
RSpec.describe WithdrawApplication do
  ...
  describe '#save!' do
    ...
    it 'sends a notification email to the training provider and ratifying provider', :sidekiq do
      ...
      expect(training_provider_email['rails-mail-template'].first.value).to eq('application_withdrawn')
      expect(ratifying_provider_email['rails-mail-template'].first.value).to eq('application_withdrawn')
    end
  end
end
