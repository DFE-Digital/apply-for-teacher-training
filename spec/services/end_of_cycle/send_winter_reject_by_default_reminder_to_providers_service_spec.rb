...
RSpec.describe EndOfCycle::SendWinterRejectByDefaultReminderToProvidersService do
  describe '#call' do
    context 'user was notified in a previous cycle', time: after_apply_deadline do
      ...
      before do
        create(:chaser_sent,
               chased: user,
               chaser_type: :respond_to_applications_before_winter_reject_by_default_date,
      ...
      it 'sends an email' do
        expect { described_class.new(provider).call }
          .to have_enqueued_mail(
            ProviderMailer, :respond_to_applications_before_winter_reject_by_default_date
          )
      end
    end
    ...
    context 'user has already been sent the email', time: after_apply_deadline do
      ...
      before do
        create(:chaser_sent, chased: user, chaser_type: :respond_to_applications_before_winter_reject_by_default_date)
      end
