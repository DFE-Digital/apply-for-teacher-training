    ...
    context 'when a english proficiency is not given' do
      context 'when the qualification statuses is "qualification_not_needed"' do
        it 'creates a new published english proficiency with the qualification_not_needed attribute toggled' do
          ...
          expect{ call }.to change(application_form.english_proficiencies.draft, :count).by(0)

          new_english_proficiency = application_form.english_proficiency
          expect(new_english_proficiency.qualification_not_needed).to eq(true)
          expect(new_english_proficiency.no_qualification).to eq(false)
          expect(new_english_proficiency.degree_taught_in_english).to eq(false)
          expect(new_english_proficiency.has_qualification).to eq(false)
          expect(new_english_proficiency.draft).to eq(false)
        end
      end
      ...
      context 'when the qualification statuses is "no_qualification"' do
        ...
        it 'creates a new draft english proficiency with the no_qualification attribute toggled' do
          ...
          expect(new_english_proficiency.qualification_not_needed).to eq(false)
          expect(new_english_proficiency.no_qualification).to eq(true)
          expect(new_english_proficiency.degree_taught_in_english).to eq(false)
          expect(new_english_proficiency.has_qualification).to eq(false)
          expect(new_english_proficiency.draft).to eq(true)
        end
      ...
      context 'when the qualification statuses is "degree_taught_in_english"' do
        ...
        it 'creates a new draft english proficiency with the degree_taught_in_english attribute toggled' do
          ...
          expect(new_english_proficiency.qualification_not_needed).to eq(false)
          expect(new_english_proficiency.no_qualification).to eq(false)
          expect(new_english_proficiency.degree_taught_in_english).to eq(true)
          expect(new_english_proficiency.has_qualification).to eq(false)
          expect(new_english_proficiency.draft).to eq(true)
        end
      ...
      context 'when the qualification statuses is "has_qualification"' do
        ...
        it 'creates a new draft english proficiency with the has_qualification attribute toggled' do
          ...
          expect(new_english_proficiency.no_qualification).to eq(false)
          expect(new_english_proficiency.degree_taught_in_english).to eq(false)
          expect(new_english_proficiency.has_qualification).to eq(true)
          expect(new_english_proficiency.draft).to eq(true)
        end
    ...
    context 'when an english proficiency is given' do
      context 'when the english proficiency is in a draft state' do
        ...
        it 'updates and published the given english proficiency' do
          expect{ call }.to change(application_form.english_proficiencies, :count).by(0)
          expect(english_proficiency.qualification_not_needed).to eq(true)
          expect(english_proficiency.no_qualification).to eq(false)
          expect(english_proficiency.degree_taught_in_english).to eq(false)
          expect(english_proficiency.has_qualification).to eq(false)
          expect(english_proficiency.draft).to eq(false)
        end
        ...
        context 'when the qualification statuses is "no_qualification"' do
          ...
          it 'updates the given english proficiency' do
            expect{ call }.to change(application_form.english_proficiencies, :count).by(0)
            expect(english_proficiency.qualification_not_needed).to eq(false)
            expect(english_proficiency.no_qualification).to eq(true)
            expect(english_proficiency.degree_taught_in_english).to eq(false)
            expect(english_proficiency.has_qualification).to eq(false)
            expect(english_proficiency.draft).to eq(true)
          end
        end
