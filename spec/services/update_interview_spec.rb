...
RSpec.describe UpdateInterview do
  ...
  describe '#save!' do
    ...
    it 'creates an audit entry and sends an email', :sidekiq, :with_audited do
      ...
      expect(ActionMailer::Base.deliveries.first['rails-mail-template'].first.value).to eq('interview_updated')
    end
  ...
  context 'if interview validations fail', :sidekiq do
    ...
    it 'raises a ValidationException, does not send emails' do
      ...
      expect(ActionMailer::Base.deliveries.map { |d| d['rails-mail-template'].first.value }).not_to include('interview_updated')
    end
  end
