...
RSpec.describe UpdateInterview do
  ...
  describe '#save!' do
    ...
    it 'creates an audit entry and sends an email', :with_audited do
      described_class.new(**service_params).save!
  ...
  context 'called via the API' do
    ...
    it 'accepts a vendor_api_user', :with_audited do
      described_class.new(**service_params).save!
  ...
  context 'if interview workflow constraints fail' do
  ...
  context 'if the update changes no fields' do
    let(:service_params) do
      {
        actor: provider_user,
        provider:,
