...
RSpec.describe DeclineOffer do
  ...
  context 'when accepted_offer is true' do
    it 'notifies the training provider and ratifying provider that the offer was declined automatically', :run_jobs do
  ...
  context 'when accepted_offer is false by default' do
    it 'notifies the training provider and ratifying provider that the candidate declined the offer', :run_jobs do
      training_provider = create(:provider)
      training_provider_user = create(:provider_user, :with_notifications_enabled, providers: [training_provider])
