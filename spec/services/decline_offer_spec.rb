...
RSpec.describe DeclineOffer do
  ...
  it 'sends a notification email to the training provider and ratifying provider', :sidekiq do
    ...
    expect(training_provider_email['rails-mail-template'].first.value).to eq('declined')
    expect(ratifying_provider_email['rails-mail-template'].first.value).to eq('declined')
  end
end
