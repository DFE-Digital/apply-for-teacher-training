  ...
  describe '#change' do
    it 'migrates the filters from provider_user to provider_user_filters' do
      ...
      expect { described_class.new.change }.to change {ProviderUserFilter.count}.from(0).to(2)
        .and change { user_with_filters_1.filters.find_candidates_all.last&.filters }.from(nil).to(location_filter)
        .and change { user_with_filters_2.filters.find_candidates_all.last&.filters }.from(nil).to(location_and_subject_filter)
    end
  end
end
