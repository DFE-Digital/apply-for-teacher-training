...
RSpec.describe DataMigrations::AddAllRecruitmentCycleTimetablesToDatabase do
  it 'adds all recruitment cycles to database' do
    ...
    expect(RealCycleTimetable.pluck(:recruitment_cycle_year))
      .to contain_exactly(2019, 2020, 2021, 2022, 2023, 2024, 2025, 2026, 2027)
  end

  it 'classifies the timetables as real' do
    described_class.new.change
    expect(RealCycleTimetable.where(type: 'RealCycleTimetable').pluck(:recruitment_cycle_year))
      .to contain_exactly(2019, 2020, 2021, 2022, 2023, 2024, 2025, 2026, 2027)
  end

  it 'handles holidays as expected' do
    described_class.new.change
    year_without_holidays = RealCycleTimetable.find_by(recruitment_cycle_year: 2020)
    year_with_holidays = RealCycleTimetable.find_by(recruitment_cycle_year: 2021)
