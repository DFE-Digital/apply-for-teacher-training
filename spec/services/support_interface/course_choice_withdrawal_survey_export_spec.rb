...
RSpec.describe SupportInterface::CourseChoiceWithdrawalSurveyExport do
  ...
  describe '#call' do
    it 'returns a hash of candidates course choice withdrawal survey answers' do
      ...
      expect(described_class.new.data_for_export).to contain_exactly(return_expected_hash(application_choice1), return_expected_hash(application_choice2), return_expected_hash(application_choice3))
    end
  end
