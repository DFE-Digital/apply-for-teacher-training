...
RSpec.describe SupportInterface::NotesExport do
  describe 'documentation' do
    before do
      ...
      application_choice = create(:application_choice, candidate:, course_option: create(:course_option, course:))
      provider_user = create(:provider_user, providers: [training_provider])

      create(:note, application_choice:, user: provider_user)
    end
  ...
  describe 'data_for_export' do
    it 'returns a hash of notes data' do
      ...
      application_choice2 = create(:application_choice, candidate:, current_course_option: course_option)
      ...
      create(:provider_relationship_permissions, training_provider: create(:provider, code: 'EF3'), ratifying_provider: training_provider)
    ...
    it 'omits provider org data if the provider user is no longer related to the offered course' do
      ...
      application_choice = create(:application_choice, candidate:, course_option: create(:course_option, course:), current_course_option: create(:course_option, course: current_course))
      ...
      create(:provider_relationship_permissions, training_provider: training_provider1, ratifying_provider:)
      create(:provider_relationship_permissions, training_provider: training_provider2, ratifying_provider:)

      note1 = create(:note, application_choice:, user: provider_user1)
      note2 = create(:note, application_choice:, user: provider_user2)
