  ...
  describe '#call' do
    ...
    it 'returns an array of hashes with the correct keys' do
      ...
      expect(returned_array_of_hashes.first.keys).to \
        eq %w[candidate_id first_name last_name btrim date_of_birth email_address]
    end
    ...
    it 'returns an array of hashes the correct values' do
      ...
      expect(returned_array_of_hashes.first['btrim']).to eq('W6 9BH')
      ...
      expect(returned_array_of_hashes.second['btrim']).to eq('W6 9BH')
