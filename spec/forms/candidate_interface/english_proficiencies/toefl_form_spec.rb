...
RSpec.describe CandidateInterface::EnglishProficiencies::ToeflForm, type: :model do
  ...
  describe 'validations' do
    ...
    it 'is invalid if award year is before toefl was introduced' do
      ...
      expect(form).not_to be_valid
      expect(form.errors.full_messages).to eq ['Award year Assessment year cannot be before 1964']
    end
  ...
  describe '#fill' do
    ...
    it 'assigns the qualification attributes to the form' do
      ...
      expect(form.registration_number).to eq(efl_qualification.registration_number)
      expect(form.total_score).to eq(efl_qualification.total_score)
    ...
    context "when the efl qualification is not an ToeflQualification" do
      ...
      it 'does not assign the qualification attributes to the form' do
        form = valid_form.fill
        expect(form.registration_number).to be_nil
        expect(form.total_score).to be_nil
