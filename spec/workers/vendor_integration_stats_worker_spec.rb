...
RSpec.describe VendorIntegrationStatsWorker do
  describe '#perform' do
    ...
    it 'raises an error if the relevant environment variable is not set' do
      ClimateControl.modify TRIBAL_INTEGRATION_STATS_SLACK_URL: nil do
        expect { worker.perform('tribal') }.to raise_error(KeyError)
      end
    end
  ...
  describe 'SlackReport' do
    ...
    it 'generates a text-based report for sending to Slack' do
      ...
      expect(report).to include('Never connected')
      expect(report).to include('No sync in the last 24h')
      expect(report).to include('No decisions in the last 7 days')
      expect(report).to include('Providers with errors')
    end
  end
end
