...
RSpec.describe CandidateMailers::SendWithdrawnOnRequestEmailWorker do
  describe '#perform' do
    ...
    it 'sends the application_withdrawn_on_request email to the candidate' do
      ...
      expect(CandidateMailer).to have_received(:application_withdrawn_on_request).with(application_choice, nil)
    end

    context 'when recommended courses URL is provided' do
      let(:recommended_courses_url) { 'https://example.com/recommended-courses' }

      it 'sends the application_withdrawn_on_request email with the recommended courses URL' do
        application_choice = create(:application_choice, status: :rejected)

        described_class.new.perform(application_choice.id)

        expect(CandidateMailer).to have_received(:application_withdrawn_on_request).with(application_choice, 'https://example.com/recommended-courses')
      end
    end
  end
end
