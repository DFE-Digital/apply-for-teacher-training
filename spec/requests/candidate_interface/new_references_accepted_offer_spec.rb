...
RSpec.describe 'Candidate Interface - Redirects acepted offer', type: :request do
  ...
  context 'when candidate accepted an offer' do
    ...
    context 'when new references is active and cycle year is 2022' do
      ...
      it 'redirects to the post offer dashboard' do
        application_form = create(:application_form, submitted_at: Time.zone.now, recruitment_cycle_year: 2022, candidate: candidate)
        create(:application_choice, :with_accepted_offer, application_form: application_form)
        create(:application_choice, :with_withdrawn_offer, application_form: application_form)
        create(:application_choice, :with_withdrawn_offer, application_form: application_form)
        get candidate_interface_application_complete_path
    ...
    context 'when new references is not active' do
      ...
      it 'does not redirect to the post offer dashboard' do
        application_form = create(:application_form, recruitment_cycle_year: 2022, candidate: candidate)
        create(:application_choice, :with_accepted_offer, application_form: application_form)
        create(:application_choice, :with_withdrawn_offer, application_form: application_form)
        create(:application_choice, :with_withdrawn_offer, application_form: application_form)
        get candidate_interface_application_complete_path
