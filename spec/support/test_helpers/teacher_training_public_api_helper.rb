module TeacherTrainingPublicAPIHelper
  ...
  def stub_teacher_training_api_providers(recruitment_cycle_year: current_year, specified_attributes: [], filter_option: nil)
    scope = stub_request(
      :get,
  ...
  def stub_teacher_training_api_providers_with_multiple_pages(recruitment_cycle_year: current_year)
    [1, 2, 3].each do |page_number|
      stub_pagination_request(recruitment_cycle_year, page_number, paginated_response(page_number))
    end
  end

  def stub_teacher_training_api_provider(provider_code:, recruitment_cycle_year: current_year, specified_attributes: [], filter_option: nil)
  ...
  def stub_teacher_training_api_provider_404(provider_code:, recruitment_cycle_year: current_year)
    stub_404("#{ENV.fetch('TEACHER_TRAINING_API_BASE_URL')}/recruitment_cycles/#{recruitment_cycle_year}/providers/#{provider_code}")
  end

  def stub_teacher_training_api_courses_404(provider_code:, recruitment_cycle_year: current_year)
  ...
  def stub_teacher_training_api_course_404(provider_code:, course_code:, recruitment_cycle_year: current_year)
  ...
  def stubbed_recruitment_cycle_year
    @stubbed_recruitment_cycle_year || current_year
  end
