...
RSpec.describe DfE::Bigquery::RegionalEdiMetrics do
  ...
  describe '.data' do
    ...
    before do
      stub_bigquery_regional_edi_request(rows:)
    end
    ...
    it 'assigns the attributes for the application metrics', :aggregate_failures do
      expect(region_statistics.first.nonregion_filter).to eq 'Prefer not to say'
      expect(region_statistics.first.nonregion_filter_category).to eq 'Sex'
  ...
  describe described_class::Result do
    let(:result) { described_class.new({ region_filter: 'London', nonregion_filter: 'Prefer not to say' }) }
  ...
  describe 'when the query returns nil for rows' do
    ...
    let(:stub_bigquery_response) do
      stub_bigquery_regional_edi_request(result: false)
    end
  ...
  describe 'when there is an error' do
    ...
    context 'when there is more than one page' do
      before do
        stub_bigquery_regional_edi_request(page_token: true)
      end
    ...
    context 'when the query job does not complete in time' do
      before do
        stub_bigquery_regional_edi_request(job_complete: false)
      end
