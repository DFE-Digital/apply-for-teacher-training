require 'rails_helper'

RSpec.describe Publications::MonthlyStatistics::ByPrimarySpecialistSubject do
  include StatisticsTestHelper

  before { generate_statistics_test_data }

  subject(:statistics) { described_class.new.table_data }

  it 'generates correct table data' do
    expect_report_rows(column_headings: ['Subject', 'Recruited', 'Conditions pending', 'Deferred', 'Received an offer', 'Awaiting provider decisions', 'Unsuccessful', 'Total']) do
      [['Primary',                            0, 0, 0, 0, 1, 0, 1],
       ['Primary with English',               0, 1, 0, 0, 1, 0, 2],
       ['Primary with geography and history', 0, 0, 0, 1, 1, 0, 2],
       ['Primary with mathematics',           1, 0, 0, 0, 1, 0, 2]]
    end

    expect_column_totals(1, 1, 0, 1, 4, 0, 7)
  end
end
