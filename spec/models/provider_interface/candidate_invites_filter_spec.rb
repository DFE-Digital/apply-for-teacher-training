...
RSpec.describe ProviderInterface::CandidateInvitesFilter do
  ...
  let(:invite_with_opted_out_candidate) { create(:pool_invite, :sent_to_candidate, course: build(:course, provider:), candidate: opted_out_preference.candidate) }

  let(:opted_in_preference) { create(:candidate_preference, :anywhere_in_england, pool_status: 'opt_in') }
  let(:invite_with_candidate_in_pool) { create(:pool_invite, :sent_to_candidate, candidate: opted_in_preference.candidate, course: build(:course, provider:)) }
  let(:second_invite_with_candidate_in_pool) { create(:pool_invite, :sent_to_candidate, candidate: opted_in_preference.candidate, course: build(:course, provider:)) }
  ...
  context 'user does have saved filters and new filter_params are provided' do
    it 'updates the filter_params and returns filtered courses' do
      user_filter = create(:provider_user_filter, provider_user:, filters: { status: ['invited'] })
