...
RSpec.describe SupportInterface::VendorAPIMonitor do
  ...
  describe '#never_connected' do
    ...
    it 'returns only providers with no API request logs ever for a specific vendor when the vendor is provided' do
      provider_who_hasnt_connected_from_correct_vendor = create(:provider, vendor:)
      provider_who_has_connected_from_correct_vendor = create(:provider, vendor:)
  ...
  describe '#no_sync_in_24h' do
    ...
    it 'returns only providers who have connected and who have not synced in 24h for a specific vendor when provided' do
      synced_recently = create(:provider, name: 'recently', vendor:)
      failed_recently = create(:provider, name: 'failed', vendor:)
      synced_1w_ago = create(:provider, name: '1 week ago', vendor:)
      synced_2d_ago = create(:provider, name: '3 hours', vendor:)
      ...
      create(:vendor_api_request, provider: synced_1w_ago, created_at: 1.week.ago)
      create(:vendor_api_request, provider: synced_2d_ago, created_at: 2.days.ago)
      expect(monitor(vendor:).no_sync_in_24h.map(&:id)).to eq [failed_recently.id, synced_2d_ago.id, synced_1w_ago.id]
    end
  end
  ...
  describe '#no_sync_in_7d' do
    ...
    it 'returns only providers who have connected and who have not synced in 7d for a specific vendor when provided' do
      ...
      failed_recently = create(:provider, name: 'failed', vendor:)
      synced_1w_ago = create(:provider, name: '1 week ago', vendor:)
      synced_2w_ago = create(:provider, name: '2 weeks ago', vendor:)
      failed_recently_with_other_vendor = create(:provider, :with_vendor, name: 'failed')
  ...
  describe '#no_decisions_in_7d' do
    ...
    it 'returns only providers who have connected and who have not made decisions in 7 days for a given vendor' do
      _never_connected = create(:provider, name: 'never', vendor:)
      decided_recently = create(:provider, name: 'recently', vendor:)
      failed_recently = create(:provider, name: 'failed', vendor:)
      decided_over_2w_ago = create(:provider, name: '2 weeks', vendor:)
      decided_over_7d_ago = create(:provider, name: '7 days', vendor:)
      ...
      expect(monitor(vendor:).no_decisions_in_7d.map(&:id)).to eq [failed_recently.id, decided_over_7d_ago.id, decided_over_2w_ago.id]
    end
  end
  ...
  describe '#providers_with_errors' do
    ...
    it 'returns structs with providers who have connected and who have caused errors in the last 7 days for a given vendor' do
      _never_connected = create(:provider, :with_vendor, name: 'never')
      errored_recently = create(:provider, name: 'recently', vendor:)
      ...
      expect(monitor(vendor:).providers_with_errors.size).to eq 1
      expect(monitor(vendor:).providers_with_errors.first.id).to eq errored_recently.id
      expect(monitor(vendor:).providers_with_errors.first.error_count).to be 1
      expect(monitor(vendor:).providers_with_errors.first.request_count).to be 2
      expect(monitor(vendor:).providers_with_errors.first.error_rate).to eq 50.0
    end
  end
end
