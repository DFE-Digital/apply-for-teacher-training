...
task :preprocess_tad_data, [:tad_data_file_name] => [:environment] do |_t, args|
  ...
  File.write(args[:tad_data_file_name], tad_data.map { |subject, v| [subject.downcase.gsub('&', 'and').parameterize.underscore.to_sym, v.map { |status, ids| raise "cannot find #{status}" unless TAD_STATUS_MAPPINGS[status.downcase.to_sym]; [TAD_STATUS_MAPPINGS[status.downcase.to_sym], ids.map { |id| id.is_a?(Array) ? id.first : id }]}.to_h] }.to_h.to_json)
end
...
task :compare_ministerial_reports, [:bat_data_file_name, :tad_data_file_name] => :environment do |_t, args|
  ...
  puts diff_data.to_json(indent: 2)
end
