module CandidateInterface
  class AfterDeadlineContentComponent < ViewComponent::Base
    ...
    def cycle_opening_dates_content
      [].tap do |collection|
        if before_find_opens?
          collection << t('.when_find_opens', find_opens_at: find_opens_at.to_fs(:govuk_date_time_time_first))
        end

        collection << t('.when_apply_opens', apply_opens_at: apply_opens_at.to_fs(:govuk_date_time_time_first))
      end.join(' ')
    end
