class CandidateInterface::ApplicationChoices::AfterDeadlineContentComponent < ApplicationComponent
  ...
  def find_opens_text
    if before_find_opens?
      t(
        'after_deadline_content_component.when_find_opens_html',
        find_link: govuk_link_to(t('after_deadline_content_component.find_link_text'), t('find_teacher_training.production_url')),
        find_opens_at: find_opens_at.to_fs(:govuk_date_time_time_first),
      )
    else
      t(
        'after_deadline_content_component.find_has_opened_html',
        find_link: govuk_link_to(t('after_deadline_content_component.find_link_text'), t('find_teacher_training.production_url')),
      )
    end
  end
  ...
  def apply_opens_text
    t(
      'after_deadline_content_component.when_apply_opens',
  ...
  def reject_by_default_warning_text
    if application_choices.any?(&:decision_pending?)
      t(
        'after_deadline_content_component.reject_by_default_warning_text_html',
  ...
  def reject_by_default_explanation
    if application_choices.any?(&:rejected_by_default?)
      t(
        'after_deadline_content_component.reject_by_default_explanation',
  ...
  def decline_by_default_warning_text
    if application_choices.any?(&:offer?)
      t(
        'after_deadline_content_component.decline_by_default_warning_html',
  ...
  def decline_by_default_explanation
    if application_choices.any?(&:declined_by_default?)
      t(
        'after_deadline_content_component.decline_by_default_explanation',
