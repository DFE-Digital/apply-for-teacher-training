class CandidateMailer < ApplicationMailer
  ...
  def initial_invite_chaser(pool_invite)
    @pool_invite = pool_invite
    @preferences_url = candidate_preferences_link(pool_invite.candidate)
    @invite_url = edit_candidate_interface_invite_url(pool_invite)
    @application_form = pool_invite.application_form
  ...
  def pool_opt_out(application_form)
    @update_preferences_url = candidate_interface_draft_preference_publish_preferences_url(
      application_form.published_preference,
    )
  ...
  def pool_re_opt_in(application_form)
    @share_details_url = candidate_interface_share_details_url
    @update_preferences_url = candidate_interface_draft_preference_publish_preferences_url(
      application_form.published_preference,
    )
    ...
    email_for_candidate(
      application_form,
      subject: I18n.t!('candidate_mailer.pool_re_opt_in.subject'),
      layout: false,
    )
  end
