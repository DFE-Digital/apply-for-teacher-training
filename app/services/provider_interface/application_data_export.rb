    ...
    class << self
      def export_row(application_choice)
...

        {
          ...
          'address' => address(application.application_form),
      ...
      def address(application)
        %i[address_line1 address_line2 address_line3 address_line4].map { |field| application.send(field) }.compact.join(', ')
      end
    end
  end
end
