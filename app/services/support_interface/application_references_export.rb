    ...
    def data_for_export
      ...
      data_for_export = application_forms.map do |application_form|
        ...
        application_form.application_references.map.with_index(1) do |reference, index|
          output["ref_#{index}_type".to_sym] = reference.referee_type
          output["ref_#{index}_state".to_sym] = reference.feedback_status
          output["ref_#{index}_requested_at".to_sym] = reference.requested_at
          output["ref_#{index}_received_at".to_sym] = received_at(reference)
        end
