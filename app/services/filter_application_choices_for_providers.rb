class FilterApplicationChoicesForProviders
  ...
  private
    ...
    def course_type(application_choices, course_type_filter)
      return application_choices if course_type_filter.blank? || course_type_filter.compact_blank.sort == ['non_tda', 'tda']

      if course_type_filter.include?('tda')
        application_choices.joins(:course).where("courses.qualifications ? 'tda'")
      else
        application_choices.joins(:course).where.not("courses.qualifications ? 'tda' ")
      end
    end
