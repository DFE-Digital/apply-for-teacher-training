  ...
  def as_slack_message
    <<~MARKDOWN
      *Today on Apply*
      ...
      :#{%w[man woman].sample}man-tipping-hand: #{pluralize(offers_made(today), 'offer')} made | #{offers_made(this_day_last_year)} last cycle
      :#{%w[man woman].sample}man-tipping-hand: #{pluralize(offers_accepted(today), 'offer')} accepted | #{offers_accepted(this_day_last_year)} last cycle
      :#{%w[man woman].sample}man-gesturing-no: #{pluralize(rejections_issued(today), 'rejection')} issued#{rejections_issued(today).positive? ? ", of which #{pluralize(rbd_count(today), 'was')} RBD" : nil} | #{rejections_issued(this_day_last_year)} last cycle
