  ...
  class MonthlyStatistics::Timetable
    ...
    def next_publication_date
      if unpublished_schedules.blank?
        MonthlyStatistics::Timetable.new(recruitment_cycle_timetable: recruitment_cycle_timetable.relative_next_timetable).next_publication_date
      else
    ...
    def unpublished_schedules
      schedules.filter do |schedule|
        schedule.publication_date.after? Time.zone.now
      end
    end

    def generated_schedules
      schedules.filter do |schedule|
        schedule.generation_date.before? Time.zone.now
      end
    end
    ...
    def generation_today_schedule
      schedules.find do |schedule|
        schedule.generation_date.to_date == Time.zone.now.to_date
      end
    end
