module ProviderInterface
  class OfferWizard
    ...
    def next_step(step = current_step)
      ...
      current_step = STEPS[decision.to_sym][index + 1]

      return save_and_go_to_next_step(current_step) if current_step.eql?(:providers) && available_providers.one?
      return save_and_go_to_next_step(current_step) if current_step.eql?(:courses) && available_courses.one?
      return save_and_go_to_next_step(current_step) if current_step.eql?(:study_modes) && available_study_modes.one?
      return save_and_go_to_next_step(current_step) if current_step.eql?(:locations) && available_course_options.one?
  ...
  private
    ...
    def available_study_modes
      Course.find_by(provider_id: provider_id).available_study_modes_from_options
    end
