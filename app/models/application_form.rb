class ApplicationForm < ApplicationRecord
  ...
  def self.test(id)
    ...
    choice = form.application_choices.last
    {
      candidate_id: form.candidate_id,
      # account_recovery_requests
      form_id: form.id,
      CandidatePoolApplication: form.candidate_pool_application&.id, # not in analytics
      # one login auths
      PoolInvite: form.published_invites&.ids,
      # possible previous_teacher_trainings not in analytics
      # session_errors not in analytics
      # sessions not in analytics
      ApplicationReference: form.application_references&.ids,
      # account_recovery_request_codes not in analytics
      # adviser_sign_up_requests
      ApplicationChoice: form.application_choices&.ids,
      application_feedback: form.application_feedback&.ids,
      ApplicationQualification: form.application_qualifications&.ids,
      Email: form.emails&.ids, # not in analytics
      # pool_eligible_application_forms
      # account_recovery_request_codes
      PreviousTeacherTraining: form.previous_teacher_trainings&.ids,
      # Provider pool actions - not in analytics
      EmailClick: form.emails.last.email_clicks&.ids,
      Interview: form.application_choices.last.interviews&.ids,
      Note: form.application_choices.last.notes&.ids,
      Offer: form.application_choices.last.offer&.id,
      ReferenceToken: form.application_references.last.reference_tokens&.ids,
      # rejection_feedbacks
      WithdrawalReason: choice.withdrawal_reasons&.ids,
      OfferCondition: form.application_choices.last.offer.conditions&.ids,

      # rest are not in the SQL cascade query
      ...
      EnglishProficiency: form.english_proficiency&.id,
      ChoiceApplicationWorkExperience: choice.work_experiences&.ids,
      CHOICEApplicationVolunteeringExperience: choice.volunteering_experiences&.ids,
      CHOICEApplicationWorkHistoryBreak: choice.work_history_breaks&.ids,
    }
    { 'candidates' => [59],
      'account_recovery_requests' => [],
      'application_forms' => [105, 58],
      'candidate_pool_applications' => [33],
      'one_login_auths' => [],
      'pool_invites' => [1],
      'possible_previous_teacher_trainings' => [],
      'session_errors' => [],
      'sessions' => [],
      '"references"' => [232, 409, 410, 411, 412, 229, 230, 231],
      'account_recovery_request_codes' => [],
      'adviser_sign_up_requests' => [],
      'application_choices' => [96, 97, 98, 99, 95],
      'application_feedback' => [3],
      'application_qualifications' => [356, 357, 359, 360, 361, 628, 629, 630, 631, 632, 633, 358],
      'emails' => [378, 379, 380, 381, 382, 383, 384, 385, 414, 415],
      'pool_eligible_application_forms' => [],
      'pool_invite_decline_reasons' => [],
      'previous_teacher_trainings' => [58, 105],
      'provider_pool_actions' => [1],
      'email_clicks' => [2],
      'interviews' => [6],
      'notes' => [119],
      'offers' => [39],
      'reference_tokens' => [36],
      'rejection_feedbacks' => [],
      'withdrawal_reasons' => [5],
      'offer_conditions' => [39, 40, 41] }
  end
