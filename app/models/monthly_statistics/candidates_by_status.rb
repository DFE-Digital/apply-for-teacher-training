module MonthlyStatistics
  ...
  private
    ...
    def group_query
      apply_1_status_counts = ApplicationForm
        .joins(:application_choices)
        .where(recruitment_cycle_year: RecruitmentCycle.current_year)
        .where('NOT EXISTS (SELECT 1 FROM application_forms as subsequent_application_forms where application_forms.id = subsequent_application_forms.previous_application_form_id)')
        .group('application_choices.status', 'phase')
        .count

      apply_2_status_counts = ApplicationForm
        .joins(:application_choices)
        .where(recruitment_cycle_year: RecruitmentCycle.current_year)
        .where('EXISTS (SELECT 1 FROM application_forms as subsequent_application_forms where application_forms.id = subsequent_application_forms.previous_application_form_id)')
        .group('application_choices.status', 'phase')
        .count

      apply_1_status_counts.merge!(apply_2_status_counts)
    end
