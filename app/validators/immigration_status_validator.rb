class ImmigrationStatusValidator < ActiveModel::EachValidator
  ...
  VISAS_THAT_REQUIRE_SPONSORSHIP = %w[student_visa skilled_worker_visa].freeze
  ...
  def validate_each(record, attribute, application_choice)
    return if did_not_add_nationality_yet?(application_choice) ||
              ...
              not_require_sponsorship?(application_choice) ||
  ...
  def not_require_sponsorship?(application_choice)
    application_choice.application_form.right_to_work_or_study_yes? &&
      !application_choice.application_form.immigration_status.in?(VISAS_THAT_REQUIRE_SPONSORSHIP)
  end
