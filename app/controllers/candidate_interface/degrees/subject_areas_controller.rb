module CandidateInterface
  module Degrees
    class SubjectAreasController < BaseController
      def new
        degree_attrs = { application_form_id: current_application.id, id: params[:id] }.compact
        @form = Degrees::SubjectAreaForm.new(degree_store, degree_attrs)
        @form.save_state!
      end

      def update
        @form = Degrees::SubjectAreaForm.new(degree_store, subject_params)
        if @form.valid?
          @form.save_state!
          next_step!
        else
