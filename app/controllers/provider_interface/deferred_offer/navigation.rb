  ...
  private
    ...
    def next_step_path(deferred_offer_confirmation)
      ...
      if deferred_offer_confirmation.errors.include?(:course)
        return provider_interface_deferred_offer_course_path(application_choice, return_to_review? ? { return_to: 'review' } : nil)
      end

      if deferred_offer_confirmation.errors.include?(:location)
        return provider_interface_deferred_offer_location_path(application_choice, return_to_review? ? { return_to: 'review' } : nil)
      end

      if deferred_offer_confirmation.errors.include?(:study_mode)
        return provider_interface_deferred_offer_study_mode_path(application_choice, return_to_review? ? { return_to: 'review' } : nil)
      end
